	...
	return function($) {
		...
		+function($) {
			...
			Button.prototype.setState = function(state) {
				...
				setTimeout($.proxy(function() {
					$el[val](data[state] == null ? this.options[state]
							: data[state])
			...
			Button.prototype.toggle = function() {
				...
				if ($parent.length) {
					...
					if (changed)
						$input.prop('checked',
								!this.$element.hasClass('active')).trigger(
								'change')
				} else {
			...
			function Plugin(option) {
				return this.each(function() {
					...
					if (!data)
						$this.data('bs.button', (data = new Button(this,
								options)))
		...
		+function($) {
			...
			Carousel.prototype.to = function(pos) {
				...
				var activeIndex = this
						.getItemIndex(this.$active = this.$element
								.find('.item.active'))
			...
			function Plugin(option) {
				return this.each(function() {
					...
					if (!data)
						$this.data('bs.carousel', (data = new Carousel(this,
								options)))
			...
			$(document).on('click.bs.carousel.data-api', '[data-slide]',
					clickHandler).on('click.bs.carousel.data-api',
		...
		+function($) {
			...
			Collapse.prototype.show = function() {
				...
				var actives = this.$parent
						&& this.$parent.children('.panel').children(
								'.in, .collapsing')
			...
			Collapse.prototype.hide = function() {
				...
				var complete = function() {
					...
					this.$element.removeClass('collapsing')
							.addClass('collapse').trigger('hidden.bs.collapse')
				}
			...
			Collapse.prototype.addAriaAndCollapsedClass = function($element,
				...
				$trigger.toggleClass('collapsed', !isOpen).attr(
			...
			function Plugin(option) {
				return this.each(function() {
					...
					if (!data)
						$this.data('bs.collapse', (data = new Collapse(this,
								options)))
					if (typeof option == 'string')
						data[option]()
				})
		...
		+function($) {
			...
			Dropdown.prototype.keydown = function(e) {
				...
				var $items = $parent.find('[role="menu"]' + desc
						+ ', [role="listbox"]' + desc)
			...
			$(document).on('click.bs.dropdown.data-api', clearMenus).on(
					...
					Dropdown.prototype.toggle).on(
					'keydown.bs.dropdown.data-api', toggle,
					Dropdown.prototype.keydown).on(
					'keydown.bs.dropdown.data-api', '[role="menu"]',
					Dropdown.prototype.keydown).on(
					'keydown.bs.dropdown.data-api', '[role="listbox"]',
		...
		+function($) {
			...
			var Modal = function(element, options) {
				...
				if (this.options.remote) {
					this.$element.find('.modal-content').load(
							this.options.remote, $.proxy(function() {
			...
			Modal.prototype.escape = function() {
				if (this.isShown && this.options.keyboard) {
					this.$element.on('keydown.dismiss.bs.modal', $.proxy(
							function(e) {
									...
									$
											.proxy(
													function(e) {
														...
														this.options.backdrop == 'static' ? this.$element[0]
																.focus()
																: this.hide()
													}, this))
			...
			Modal.prototype.setScrollbar = function() {
				...
				if (this.bodyIsOverflowing)
					this.$body.css('padding-right', bodyPad
							+ this.scrollbarWidth)
			}
		...
		+function($) {
			...
			Tooltip.prototype.getOptions = function(options) {
				options = $.extend({}, this.getDefaults(),
			...
			Tooltip.prototype.show = function() {
				...
				if (this.hasContent() && this.enabled) {
					...
					this.options.container ? $tip
							.appendTo(this.options.container) : $tip
					...
					if (autoPlace) {
						...
						placement = placement == 'bottom'
								&& pos.bottom + actualHeight > containerDim.bottom ? 'top'
								: placement == 'top'
										&& pos.top - actualHeight < containerDim.top ? 'bottom'
										: placement == 'right'
												&& pos.right + actualWidth > containerDim.width ? 'left'
												: placement == 'left'
														&& pos.left
																- actualWidth < containerDim.left ? 'right'
														: placement

						$tip.removeClass(orgPlacement).addClass(placement)
					}

					var calculatedOffset = this.getCalculatedOffset(placement,
							pos, actualWidth, actualHeight)
			...
			Tooltip.prototype.getViewportAdjustedDelta = function(placement,
					pos, actualWidth, actualHeight) {
				var delta = {
					top : 0,
					left : 0
			...
			Tooltip.prototype.destroy = function() {
				...
				this.hide(function() {
					that.$element.off('.' + that.type).removeData(
							'bs.' + that.type)
				})
			}
			...
			function Plugin(option) {
				return this.each(function() {
					...
					if (!data)
						$this.data('bs.tooltip', (data = new Tooltip(this,
								options)))
					if (typeof option == 'string')
						data[option]()
				})
		...
		+function($) {
			...
			Popover.prototype.getContent = function() {
				...
				return $e.attr('data-content')
						|| (typeof o.content == 'function' ? o.content
								.call($e[0]) : o.content)
			}
			...
			function Plugin(option) {
				return this.each(function() {
					...
					if (!data)
						$this.data('bs.popover', (data = new Popover(this,
								options)))
					if (typeof option == 'string')
						data[option]()
				})
		...
		+function($) {
			...
			function ScrollSpy(element, options) {
				...
				this.$scrollElement.on('scroll.bs.scrollspy', $.proxy(
						this.process, this))
				this.refresh()
				this.process()
			}
			...
			function Plugin(option) {
				return this.each(function() {
					...
					if (!data)
						$this.data('bs.scrollspy', (data = new ScrollSpy(this,
								options)))
					if (typeof option == 'string')
						data[option]()
				})
		...
		+function($) {
			...
			Tab.prototype.activate = function(element, container, callback) {
				...
				$active.length && transition ? $active.one('bsTransitionEnd',
						next).emulateTransitionEnd(Tab.TRANSITION_DURATION)
						: next()
			...
			$(document).on('click.bs.tab.data-api', '[data-toggle="tab"]',
					clickHandler).on('click.bs.tab.data-api',
		...
		+function($) {
			...
			var Affix = function(element, options) {
				...
				this.$target = $(this.options.target).on(
			...
			Affix.prototype.getState = function(scrollHeight, height,
			...
			Affix.prototype.checkPosition = function() {
				...
				if (this.affixed != affix) {
					...
					this.$element.removeClass(Affix.RESET).addClass(affixType)
							.trigger(
									affixType.replace('affix', 'affixed')
											+ '.bs.affix')
				}
